...
Maze::Maze(std::istream& mapFile)
{
  ...
  if (!cowardFlag)
    {
      std::cout << "Congratulations brave player. You climb out of the basement\n"
		<< "only to be eaten by the monster. Oh well...\n";
...
{
  std::string moveOptions = "q";
  std::cout << std::endl << (location->message) << std::endl
	    << "You can see doors around you to the ";
  if (location->north != NULL)
    {
      std::cout << "north ";
      moveOptions = moveOptions + "n";
    }
  if (location->south != NULL)
    {
      std::cout << "south ";
      moveOptions = moveOptions + "s";
    }
  ...
  while (promptAgain)
	...
	{
	  std::cout << "Please choose one of the doors, or q to quit\n";
	  promptAgain = true;
	}
      else
	{
